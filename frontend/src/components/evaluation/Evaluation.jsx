...
function Evaluation() {
  ...
  const handleClickToPageThree = (e) => {
    ...
    const result =
      INITIAL_PRICE *
      ponderation.memory[memoryRef.current.value] *
      ponderation.storage[storageRef.current.value] *
      ponderation.release[releaseRef.current.value] *
      ponderation.condition[conditionRef.current.value] *
      ponderation.brand[brandRef.current.value];
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <section className='container first-page'>
          ...
          <button className='next btn' onClick={handleClickToPageTwo}>
            Suivant
          </button>
        </section>
            ...
            <div className='left'>
              ...
              <div className='ram select-container'>
                ...
                <select name='ram' id='ram' ref={memoryRef}>
                  <option value=''></option>
                  {memory.map((a) => (
                    <option key={a.capacity} value={a.capacity}>
                      {' '}
                      {a.capacity} Go{' '}
                    </option>
                  ))}
              ...
              <div className='network select-container'>
                ...
                <select name='network' id='network' ref={networkRef}>
                  <option value=''></option>
                  {network.map((a) => (
                    <option key={a.id} value={a.network}>
                      {' '}
                      {a.network}{' '}
                    </option>
                  ))}
              ...
              <button className='submit btn' onClick={handleClickToPageThree}>
                Valider
              </button>
            </div>
            <div className='right'>
        ...
        <section className='container third-page'>
          <div className='flex-container'>
            <div className='left'>
              <h2>
                Prix conseillé pour le{' '}
                <span className='smartphone'>smartphone</span>
              </h2>
              <p className='price'>{price} €</p>
              <div className='specs'>
                <p>Modèle: {optionModel[+smartphoneObj.model - 1]?.name}</p>
